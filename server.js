...
function gateKeeper(req, res, next) {
  const parsedInfo = queryString.parse(req.get('x-username-and-password'));
  
  const user = parsedInfo.user || null;
  const pass = parsedInfo.pass || null;

   
   req.user = USERS.find(function (callback) {
 // console.log(callback);
     return callback.userName === user && callback.password === pass
   });
